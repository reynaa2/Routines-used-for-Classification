...
# newR401 = arrays(oldR401)
# new_Xrange = arrays(new_xr)
# peak_val = arrays(peak_value)

# For sending non-logged values to Kevin M., only convert the lists into arrays
newR51 = np.asarray(oldR51)
newR101 = np.asarray(oldR101)
newR151 = np.asarray(oldR151)
newR401 = np.asarray(oldR401)
new_Xrange = np.asarray(newer_xr)
peak_val = np.asarray(peak_value)
...
df.to_csv('DR14_NonLog_StatsCatalog.csv')
